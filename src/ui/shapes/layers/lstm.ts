...
export class LSTM extends ActivationLayer {
    ...
    public parameterDefaults: { [key: string]: any } = {
        units: 64,  // 参考PyTorch代码，使用64个隐藏单元
        returnSequences: false,
        dropout: 0,  // 参考PyTorch代码，不使用dropout
        timestep: 12  // 时间窗口大小（时间步数），用于时序数据预处理
        // 注意：TensorFlow.js的LSTM层不支持recurrentDropout参数
    };
    ...
    public populateParamBox(): void {
        ...
        value1.value = "64";  // 参考PyTorch代码，默认值64
        ...
        value3.value = "0";  // 参考PyTorch代码，默认不使用dropout
        ...
        value4.value = "0";  // 参考PyTorch代码，默认不使用recurrent dropout
    ...
    public generateTfjsLayer(): void {
        ...
        // 移除recurrentDropout参数（TensorFlow.js不支持）
        if (parameters.recurrentDropout !== undefined) {
            delete parameters.recurrentDropout;
        }
        ...
        console.log(`LSTM layer: parent type=${parent.layerType}, parentShape=${JSON.stringify(parentShape)}, isTimeSeries=${isTimeSeries}`);
        ...
        // 对于时序数据，特殊处理：Input层直接连接LSTM
        if (isTimeSeries && parent && parent.layerType === "Input") {
            // 时序数据：Input层shape是[timeSteps, features]（如[12, 1]）
            // LSTM可以直接处理，TensorFlow.js会自动添加批次维度
            // 关键：不要尝试任何reshape操作
            console.log("LSTM: 时序数据，Input层直接连接，跳过所有reshape");
            this.tfjsLayer = this.tfjsEmptyLayer(parameters).apply(parentLayer);
            return;
        }
        ...
        try {
            if (parentShape.length === 4) {
                // 4D输入（批次，高度，宽度，通道）- 图像数据
                const timeSteps = parentShape[1];
                const features = parentShape[2] * parentShape[3];
            ...
            console.error("LSTM layer error:", error, "parentShape:", parentShape);
            // 如果处理失败，直接应用LSTM层
            this.tfjsLayer = this.tfjsEmptyLayer(parameters).apply(parentLayer);
        }
    }
